...
{
    /// <summary>
    /// Interaction logic for MondatokDelete.xaml
    /// </summary>
    public partial class MondatokDelete : Window
    {
        private readonly HttpClient _httpClient = new HttpClient { BaseAddress = new Uri("http://localhost:5271/") };

        public MondatokDelete()
        {
            InitializeComponent();
        }
        ...
        private async void DeleteButton_Click(object sender, RoutedEventArgs e)
        {
            if (tbId.Text != "")
                try
                {
                    var response = await _httpClient.DeleteAsync($"api/Mondatok/{Properties.Settings.Default["Token"]}?torlendoId={tbId.Text}");

                    if (response.IsSuccessStatusCode)
                    {
                        MessageBox.Show("Mondat sikeresen eltávolítva!");
                        tbId.Text = "";
                        await LoadMondatok();
                    }
                    else
